...
describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    let dataAll, plainValueAll;
    for(let i = 0; i < prodItems.length; i++){
      dataAll = await prodItems[i].getProperty('data');
      plainValueAll = await dataAll.jsonValue();
      if (plainValueAll.title.length == 0) { allArePopulated = false; }
      if (plainValueAll.price.length == 0) { allArePopulated = false; }
      if (plainValueAll.image.length == 0) { allArePopulated = false; }
    }
    expect(allArePopulated).toBe(true);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let numCart = await page.$('#cart-count');
    let num = await numCart.getProperty('innerText');
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    // page.evaluate();
    let cartLocal = await page.evaluate(() => localStorage.getItem('cart'));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      expect(checkDeleteAll).toBe(true);
    }
    let numCart = await page.$('#cart-count');
    let num = await numCart.getProperty('innerText');
    expect(num['_remoteObject'].value).toBe('0');
